...
Visual presentation changes require actual-interface evidence. Include or link privacy-safe screenshots at representative terminal or viewport dimensions when the result or risk includes layout, spacing, alignment, wrapping, clipping, typography, color, focus, selection, responsive behavior, graphical composition, or another presentation-sensitive state. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may instead use exact output from the supported interface on an identifiable revision when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. Record relevant output-shaping context such as locale and terminal width. Browser, TUI, prompt, and styled-output changes remain on the screenshot path. String assertions, component-only snapshots, and helper-level renderer tests do not replace required interface evidence. For interaction or transition changes, also record the action and observed result.
...
- **Visual interface evidence:** (`Screenshot` / `Exact plain-text output` / `N/A`; use exact output only for the narrow noninteractive, unstyled text case above; if `N/A`, remove the remaining fields)
- **Tested revision, interface, and output-shaping context:** (terminal or viewport dimensions for screenshots; locale and terminal width when relevant to exact output)
