...
- **Visual interface evidence:** (`Screenshot` / `Exact production-composition evidence` / `Exact plain-text output` / `N/A`; use production-composition evidence only for a semantic-only rendered-interface change that meets the rule above, exact output only for the narrow noninteractive unstyled-text case, and `N/A` only when no user-visible interface state changed)
- **Tested revision, interface, and evidence path:** (terminal or viewport dimensions when presentation-sensitive; final visible-content construction path for production-composition evidence; locale and terminal width when relevant to exact output)
- **Screenshot, production-composition, or exact-output evidence:** (attach or link screenshots, name the exact-head production path and observed final content, or include the exact supported-interface output)
