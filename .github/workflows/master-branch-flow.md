...
|---|---|---|
| `ci.yml` | `pull_request` → `master`; `push` → `master`; `merge_group` (dormant) | Required lint + test + build on PRs and trusted post-merge cache-warming runs. The `merge_group` trigger stays wired but never fires while the merge queue is disabled. |
| `windows-tests.yml` | `pull_request` → `master`, with `ci:windows` present; relevant label additions and PR updates | Opt-in affected-scope Windows nextest and conditional plugin-host coverage, outside the required gate. |
