...
cli-doctor-bootstrap-file-truncated-compact = [{$alias}] {$file} : sur les tours de la boucle agent et des canaux qui l'injectent, le plafond par fichier conserve {$retained} caractères sur {$total} ({$discarded} ignorés, avant le budget de tout le prompt). compact_context est activé pour cet agent (le réglage par défaut) et limite chaque fichier d'amorçage à {$limit} caractères. Définissez `compact_context = false` dans `[runtime_profiles.{$profile}]` ou raccourcissez le fichier.
