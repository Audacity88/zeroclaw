...
cli-doctor-bootstrap-file-truncated-compact = [{$alias}] {$file}：このファイルを注入するエージェントループおよびチャンネルのターンでは、ファイルごとの上限により {$total} 文字中 {$retained} 文字が保持されます（{$discarded} 文字を破棄、プロンプト全体の予算より前の段階）。このエージェントでは compact_context が有効です（既定値）。各ブートストラップファイルは {$limit} 文字に制限されます。`[runtime_profiles.{$profile}]` で `compact_context = false` を設定するか、ファイルを短くしてください。
