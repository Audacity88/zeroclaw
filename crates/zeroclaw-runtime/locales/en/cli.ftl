...
cli-doctor-bootstrap-file-truncated-compact = [{$alias}] {$file}: on agent-loop and channel turns that inject it, the per-file cap retains {$retained} of {$total} chars ({$discarded} discarded, before the whole-prompt budget). compact_context is on for this agent (the default) and caps each bootstrap file at {$limit} chars. Set `compact_context = false` in `[runtime_profiles.{$profile}]`, or shorten the file.
