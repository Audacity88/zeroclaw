...
cli-doctor-bootstrap-file-truncated-compact = [{$alias}] {$file}：在注入该文件的智能体循环与通道轮次中，每文件上限保留 {$total} 个字符中的 {$retained} 个（丢弃 {$discarded} 个，先于整个提示词预算）。该智能体的 compact_context 为开启状态（默认值），每个引导文件的上限为 {$limit} 个字符。请在 `[runtime_profiles.{$profile}]` 中设置 `compact_context = false`，或缩短该文件。
