...
| Pre-turn tool-elicitation hints in `src/agent/agent.rs`, `src/agent/loop_.rs`, `src/agent/tool_execution.rs`, and `src/agent/turn/{elicitation,mod,tool_specs}.rs`, limited to the default-off, admitted-channel turn flow proposed in #10325; no new tool execution or routing authority | Agent-loop owner in the planned `zeroclaw-kernel` extraction | Pending Core Team approval; inactive until this separate proposal is approved and the approval is recorded here | Review at the agent-loop extraction design review, or before widening trigger sources, retained hint state, or tool execution authority |
| Prefix-fingerprint instrumentation in `src/agent/turn/mod.rs` and `src/agent/turn/provider_call.rs`, limited to #10990: compute `system_chars`, `system_sha256`, `tools_count`, and `tools_sha256` from the finalized logical-request inputs and attach them to the existing `llm_request` event, with focused regressions | Agent-loop request-event producer in the planned `zeroclaw-kernel` extraction | Pending Core Team approval; inactive until this separate proposal is approved and the approval is recorded here | Review at the agent-loop extraction design review, or before expanding fingerprint inputs, retained state, trace access, or emission policy |
