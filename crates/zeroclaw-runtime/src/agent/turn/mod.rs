...
struct TurnState<'a> {
    ...
    history_has_trim_breadcrumb: &'a mut bool,
    synced: usize,
}
...
impl<'a> TurnState<'a> {
    ...
    fn trim_to_budget(
        ...
        let mut result = crate::agent::history_trim::trim_to_recent_turns(
            taken,
            context_token_budget,
        ...
        let (
            ...
            Err(e) => {
                ...
                let recovered = try_recover_context_overflow(
                    turn_state.history,
                    turn_state.history_has_trim_breadcrumb,
                    &e,
                    iteration,
                    event_tx.as_ref(),
                    on_delta.as_ref(),
            ...
            match action {
                crate::sop::SopRunAction::ExecuteStep {
                    ...
                    let step_output = if let Some(err) = assembly_error {
                        ...
                        let mut child_history_has_trim_breadcrumb = false;
                        ...
                        if let Some(err) = child_setup_error {
                            ...
                            let (nested_history, nested_history_has_trim_breadcrumb) = match owned {
                                Some(_) => {
                                    (&mut child_history, &mut child_history_has_trim_breadcrumb)
                                }
                                None => (&mut *history, &mut *history_has_trim_breadcrumb),
                            };
                            let step_result = crate::sop::executor::scope_step_call_sink(
                                ...
                                Box::pin(run_tool_call_loop(ToolLoop {
                                    history_has_trim_breadcrumb: Some(
                                        nested_history_has_trim_breadcrumb,
                                    ),
