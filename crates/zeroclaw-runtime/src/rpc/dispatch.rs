...
mod tests {
    ...
    async fn acp_turn_complete_and_list_acp_report_the_same_projected_count() {
        ...
        acp_store.create_session(sid, "test-agent", ws).unwrap();
        let history = vec![
        ...
        // A live ACP-mode session served by the dummy provider.
        let agent = crate::agent::agent::Agent::builder()
            .model_provider(Box::new(DummyModelProvider))
            .tools(crate::tools::scoped::ScopedToolRegistry::from_raw_for_test(
                vec![],
        ...
        // the runtime projection. The dummy turn appends the user prompt and
        // the assistant reply, so exactly two entries over the seeded base.
