                    ...
                    .await
                {
                    ...
                    Err(error) => {
                        if let Some(refusal) = error.chain().find_map(|cause| {
                            cause.downcast_ref::<zeroclaw_api::model_provider::ModelRefusalError>()
                        }) {
                            let mut events = Vec::new();
                            if let Some(usage) = refusal.usage.as_deref() {
                                events.push(Ok(StreamEvent::Usage(usage.clone())));
                            }
                            events.push(Err(StreamError::ModelRefusal(Box::new(refusal.clone()))));
                            events
                        } else {
                            vec![Err(StreamError::ModelProvider(error.to_string()))]
                        }
                    }
                }
            })
