...
/// Whether the API keeps earlier turns' thinking blocks in context for
/// `model`, rather than stripping them from finished turns server-side.
///
/// The generation decides, not the request shape: a model can take a fixed
/// budget and still keep prior turns (Opus 4.5 does), so the shape alone
/// cannot answer this. The generations that keep everything are Opus 4.5
/// and later, Sonnet 4.6 and later, and Fable 5.1 and later. Haiku and
/// Sonnet before 4.6 strip old thinking server-side, so resending finished
/// turns' blocks buys nothing there. Families the generations above do not
/// name, generations that cannot be read from the id, and ids that are not
/// Claude models all keep the stripping behaviour, which is what requests
/// did before this predicate existed.
#[must_use]
pub fn claude_keeps_prior_thinking(model: &str) -> bool {
    let lower = model.to_ascii_lowercase();
    let Some(rest) = claude_id_rest(&lower) else {
        return false;
    };
    let from_generation = |first_kept: (u32, u32)| {
        claude_generation(rest).is_some_and(|generation| generation >= first_kept)
    };
    match claude_family(rest) {
        ClaudeFamily::Opus => from_generation((4, 5)),
        ClaudeFamily::Sonnet => from_generation((4, 6)),
        ClaudeFamily::Fable => from_generation((5, 1)),
        ClaudeFamily::Haiku | ClaudeFamily::Mythos | ClaudeFamily::Other => false,
    }
}
...
mod tests {
    ...
    #[test]
    fn generations_the_thinking_docs_name_keep_prior_turns() {
        for model in [
            "claude-opus-4-5",
            "claude-opus-4-6",
            "claude-opus-4-8",
            ...
            assert!(
                claude_keeps_prior_thinking(model),
                "{model} should keep prior turns' thinking"
            );
        }
    }

    #[test]
    fn generations_that_strip_old_thinking_do_not_keep_prior_turns() {
        for model in [
            "claude-haiku-4-5",
            "claude-sonnet-4-5",
            "claude-sonnet-4-5-20250929",
            ...
            assert!(
                !claude_keeps_prior_thinking(model),
                "{model} should not claim to keep prior turns' thinking"
            );
        }
    }
