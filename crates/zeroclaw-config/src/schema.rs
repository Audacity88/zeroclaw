...
pub struct ChannelsConfig {
    ...
    #[description = "WeCom AI Bot long connection"]
...
"#;
    ...
    async fn wecom_ws_config_serde_defaults_and_secret_metadata() {
        ...
        assert!(WeComWsConfig::prop_is_secret("channels.wecom-ws.secret"));
    }
    ...
    async fn config_parses_wecom_ws_separate_from_wecom_webhook() {
        ...
        assert_eq!(
            parsed.channels.wecom.as_ref().unwrap().webhook_key,
            "webhook-key"
        );
        let ws = parsed.channels.wecom_ws.as_ref().unwrap();
