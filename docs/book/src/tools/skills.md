...
Successful updates keep the existing seven-day refresh interval. Git clone and pull have a two-second execution budget, followed by up to one second waiting for process-group or Windows job cleanup. Ordinary failures warn and fall back to audited local skills; interrupted pulls are not rolled back, and Git lock files are not removed automatically. If cleanup cannot be confirmed, that repository is skipped for the rest of the process rather than read while Git may still be writing. First-time clones use a temporary sibling and are installed only after successful completion, without replacing a concurrently created destination.

Within one process, clones and updates to the same checkout run one at a time, and a completed attempt suppresses further attempts for 60 seconds. Skipped loads do not extend that delay or freeze local skill contents. The next load after the delay may retry; no background retry is scheduled. This also limits retries when Git succeeds but the sync marker cannot be written. These budgets bound Git execution and its cleanup wait, not all filesystem operations or total agent construction time. Separate processes have independent retry state.
