...
The same no-operator boundary applies to non-shell tool calls in a bounded agentic child. The child loop creates a fresh non-interactive approval manager from the target's risk profile at the dispatch seam. A prompt-required non-shell call is denied before the tool implementation runs, while a call explicitly listed in `auto_approve` (or allowed by full autonomy) proceeds. Built-in shell has separate command-policy enforcement; this approval rule does not replace it. The inherited caller-capped registry is unchanged: approval evaluates calls that survived capability filtering; it does not grant additional tools or forward the caller's approval session. This behavior follows the non-interactive manager pattern from merged #10188 and preserves the nested-delegate boundary from merged #10601. Open #10610 covers a separate shell-approval surface and is not a prerequisite for this bounded non-shell rule.
