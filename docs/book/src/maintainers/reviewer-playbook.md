...
- Presentation-sensitive visual changes include actual-interface evidence from an identifiable revision plus screenshots with enough surrounding layout to assess the result. A semantic-only rendered-interface change may instead use exact-head production-composition evidence when it cannot affect layout, styling, clipping, wrapping, focus, selection, or interaction behavior. That evidence must exercise the final production path constructing the visible content and identify the changed state and observed result; source inspection, component-only snapshots, and helper-level assertions that stop before that path do not qualify. A concrete presentation concern restores the screenshot requirement. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may use exact output from the supported interface when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. Record relevant output-shaping context such as locale and terminal width. Interaction and transition claims also name the user action and observed result.
