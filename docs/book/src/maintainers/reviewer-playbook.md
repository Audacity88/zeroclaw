...
- Presentation-sensitive visual changes include actual-interface evidence from an identifiable revision plus screenshots with enough surrounding layout to assess the result. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may instead use exact output from the supported interface on an identifiable revision when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. Record relevant output-shaping context such as locale and terminal width. Browser, TUI, prompt, and styled-output changes remain on the screenshot path. String assertions, component-only snapshots, and helper-level renderer tests cannot replace the applicable interface proof. Interaction and transition claims also name the user action and observed result.
