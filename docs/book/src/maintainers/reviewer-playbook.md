...
| `risk:high` or `domain:security` | A concrete trust, credential, compatibility, governance, release-authority, or cross-cutting security boundary | Fast triage + deep review + rollback readiness + two independent Core Team approvals by default | Security and failure-mode checks, rollback clarity; only the [expedited unanswered-review lane](./pr-workflow.md#expedited-unanswered-review-lane) provides a standing exception |

`domain:security` remains independent from `risk:*`: use it for an effective security or trust boundary, not simply because the changed component is security-shaped. Either label triggers deep review and defaults to two independent Core Team approvals; only the [expedited unanswered-review lane](./pr-workflow.md#expedited-unanswered-review-lane) provides a standing exception. Automated review does not count as a Core Team approval.
...
Core reviewers aim to review pull requests in their area within five business days, as defined by [FND-003](../foundations/fnd-003-governance.md). If a required review action remains unanswered across five subsequent UTC business dates, evaluate the [expedited unanswered-review lane](./pr-workflow.md#expedited-unanswered-review-lane). Elapsed time is not approval and never clears a substantive objection, unresolved review thread, security concern, or other technical blocker. Review-only administrative holds and technically resolved stale verdicts require the lane's explicit approval and verified resolution. If the PR is not eligible, keep the required human review routed and use the handoff above when another qualified reviewer takes it.
