...
- A presentation-sensitive visual change includes actual-interface evidence on an identifiable revision and privacy-safe screenshots at representative terminal or viewport dimensions. A noninteractive, unstyled, deterministic plain-text CLI, stdout, stderr, or log change may instead use exact output from the supported interface on an identifiable revision when it makes no presentation-sensitive claim and no reviewer has identified a concrete presentation concern. The evidence records relevant output-shaping context such as locale and terminal width. Browser, TUI, prompt, and styled-output changes remain on the screenshot path. String assertions, component-only snapshots, helper-level renderer tests, or a statement that no interface smoke was performed do not satisfy the applicable requirement. Interaction and transition claims also include the action and observed result.
