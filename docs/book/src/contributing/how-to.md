...
A PR carrying either `risk:high` or `domain:security` needs deep review, a rollback plan matched to the change, and two independent Core Team approvals by default. After a required review action remains unanswered across five subsequent UTC business dates, a maintainer may evaluate the [expedited unanswered-review lane](../maintainers/pr-workflow.md#expedited-unanswered-review-lane); its canonical page owns every eligibility and accountability condition. Use `risk:manual` when a maintainer needs to freeze future automatic risk replacement; the label does not itself lower the review requirement.
