...
| [ADR-016](./ADR-016-inbound-authentication-and-principals.md) | proposed | Inbound authentication resolves credentials into canonical principals and permission-profile grants before privileged runtime surfaces act. |
...
ADR-016 remains proposed until canonical identity and grant resolution, native and peer-credential compatibility, RPC/WSS enforcement, OIDC and gateway integration, principal-owned session and private-memory storage, migration/recovery documentation, and internal-origin ownership rules meet the acceptance gates in the record.
