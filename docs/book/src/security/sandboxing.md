...
Firejail, Bubblewrap, and macOS Seatbelt replace the host command with a sandbox launcher. When the requested shell identity differs from the canonical executable path, these backends require a GNU-compatible `env` or `genv` with `--argv0` support on the host process's absolute `PATH` entries. The helper preserves the shell identity (for example, `sh` selecting a BusyBox applet or Bash compatibility mode) while executing the validated canonical target. ZeroClaw probes the resolved helper before preparing the sandbox command; each candidate probe is limited to two seconds. Install a supporting GNU coreutils helper separately where the system `env` lacks this option, including stock macOS installations. The helper and its libraries must also be accessible under the existing sandbox policy; ZeroClaw does not widen filesystem access or fall back to an unsandboxed command when they are unavailable. This host-helper lookup does not use the TUI-only environment overlay.

For these identity-preserving wrapped launches, canonical shell and helper paths containing `=` are rejected because `env` interprets such operands as environment assignments. Missing or unsupported helpers also produce a preparation error. Direct native execution, Landlock, and Docker's separate in-container shell selection do not require this helper.
